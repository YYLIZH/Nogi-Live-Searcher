            ...
            for (let i = 0; i < anchors.length; i++) {
                if (anchors[i].textContent == '클릭') {
                    if (String(anchors[i].getAttribute('href')).slice(-8) !== "chatroom") {
                        success = true
                        fetch(anchors[i].getAttribute('href'))
                            .then(response => {
                                return response.text()
                            })
                            .then(res => {
                                let parser = new DOMParser()
                                let doc = parser.parseFromString(res, 'text/html')
                                let src = doc.querySelector('iframe').getAttribute('src')
                                let twitch = 'https://www.twitch.tv/' + src.slice(src.indexOf("channel") + 8, src.indexOf("&"))
...
//https://player.twitch.tv/?channel=potsurito&parent=poooo.ml